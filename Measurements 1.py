...
IASkts = [245,224,191,160,128,112] # Different velocities for the measurements, corrected to SI units
IAS = 6*[0]
for i in range(len(hft)):
...
z = np.polyfit(x, y, 1)
p = np.poly1d(z)
plt.plot(x,p(x),"r--",label='trendline through measurement points')
plt.grid()
plt.ylabel('CL [-]')
plt.xlabel('Angle of attack [degrees]')
plt.axis([0, 12, 0, 1.25])
plt.legend(loc=2)
plt.show()
